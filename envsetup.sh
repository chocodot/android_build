...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^auzone_") ; then
       AUZONE_BUILD=$(echo -n $1 | sed -e 's/^auzone_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $AUZONE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       AUZONE_BUILD=
    fi
    export AUZONE_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${AUZONE_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the auzone model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch auzone_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the auzone github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var AUZONE_VERSION)
        ZIPFILE=auzone-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.auzone.device=$AUZONE_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $AUZONE_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AUZONE_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/auzone/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
